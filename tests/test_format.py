...
def test_dtype_sizes():
    assert tw.VB_HEADER.itemsize == 128
    assert tw.VD_SCAN_HEADER.itemsize == 192
    assert tw.VD_CHANNEL_HEADER.itemsize == 32
    assert tw.LINE_COUNTER.itemsize == 28
    assert tw.LINE_DTYPE.itemsize == 48
    assert tw.RAID_DIRECTORY.itemsize == 8 + 64 * 152
...
def test_has_flag_requires_every_named_bit():
    ...
    assert tw.has_flag(flags, Flag.PHASCOR).tolist() == [False, True, True]
    assert tw.has_flag(flags, Flag.PHASCOR | Flag.REFLECT).tolist() == [False, False, True]
    assert tw.has_any_flag(flags, Flag.PHASCOR | Flag.REFLECT).tolist() == [False, True, True]
...
def test_detect_version_rejects_non_twix_files():
    with pytest.raises(tw.UnsupportedVersionError):
        tw.detect_version(np.zeros(4, dtype=np.uint8))
...
def test_vb_files_are_one_synthetic_raid_entry():
    ...
    assert tw.detect_version(mm) is tw.TwixVersion.VB
    [entry] = tw.parse_raid_directory(mm, tw.TwixVersion.VB)
