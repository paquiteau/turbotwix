...
def test_protocol_buffers_are_parsed_lazily(gre_path):
    ...
    assert isinstance(dict.__getitem__(prot, "MeasYaps"), protocol.AttrDict)
...
def test_values_are_typed_from_syntax_not_from_key_prefix():
    assert protocol._cast_value("10000") == 10000
    assert protocol._cast_value("-3") == -3
    assert protocol._cast_value("1.5") == 1.5
    assert protocol._cast_value("0x20") == 32
    assert protocol._cast_value('"text"') == "text"
    assert protocol._cast_value("free_form") == "free_form"
    # A key named like a flag does not make its value a bool, and vice versa.
    assert protocol._cast_value("0") == 0
    assert protocol._cast_value("2") == 2
...
def test_identifiers_with_underscores_stay_strings():
    ...
    assert protocol._cast_value(uid) == uid
    assert protocol._cast_xprot(uid, "Double") == uid
    assert protocol._cast_xprot("1_000", "Long") == "1_000"
    # ... while real numbers still parse.
    assert protocol._cast_xprot("1.25e3", "Double") == 1250.0
    assert protocol._cast_xprot("20.000000", "Double") == 20.0
    assert protocol._cast_xprot("", "Long") is None


def test_xprot_values_use_the_declared_tag_type():
    parsed = protocol._parse_xprot(
        '<ParamLong."lA"> { 5 }\n'
        '<ParamBool."bB"> { "true" }\n'
        '<ParamBool."bC"> { "false" }\n'
...
def test_parse_buffer_strips_ascconv_section_from_xprot_remainder():
    ...
    assert "alTR" not in protocol._ASCCONV_PATTERN.sub("", buffer)

    prot = protocol._parse_buffer(buffer)
